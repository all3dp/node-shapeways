...
client.prototype.addOrder = function(params, callback) {
    var required = ["firstName", "lastName", "country", "state", "city",
        "address1", "paymentVerificationId", "paymentMethod", "shippingOption"];
    var missing = [];
    required.forEach(function(property) {
        if(!(property in params)) {
            missing.push(property);
    ...
    this.post(this.url("/orders/"), JSON.stringify(params), callback);
};

/**
 * Make an API call [GET /orders/{orderId}/v1]{@link https://developers.shapeways.com/api-reference#GET_-orders-orderId-v1}
 *
 * @method getOrderInfo
 * @memberof client
 * @instance client
 *
 * @param {number} orderId - the order to fetch information for
 * @param {client~resultsCallback} [callback] - function to call when finished
 **/
client.prototype.getOrder = function(orderId, callback) {
    this.get(this.url("/orders/" + orderId), callback);
};
